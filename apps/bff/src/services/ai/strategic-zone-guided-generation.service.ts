...
export class StrategicZoneGuidedGenerationService {
  private readonly logger = new Logger(StrategicZoneGuidedGenerationService.name);
  ...
  constructor(
    ...
    this.logger.log('Strategic Zone-Guided Generation Service initialized');
  }
  ...
  async generateZoneGuidedCandidates(request: ZoneGuidedGenerationRequest): Promise<ZoneGuidedGenerationResult> {
    ...
    this.logger.log(`Starting zone-guided generation for ${request.totalTargetCandidates} candidates across ${request.strategicZones.length} zones`);
    ...
    try {
      ...
      this.logger.log(`Zone-guided generation completed: ${totalCandidates.length} candidates in ${generationTime}ms`);
      ...
      this.logger.error('Zone-guided generation failed:', error);
  ...
  private async generateCandidatesForZones(
    ...
    for (const zone of request.strategicZones) {
      ...
      this.logger.log(`Generating ${targetCount} candidates for zone: ${zone.name}`);
      ...
      try {
        ...
        this.logger.error(`Failed to generate candidates for zone ${zone.name}:`, error);
