...
// Static validation functions (simplified for Node.js)
function validateCSSStructure(cssContent) {
  const results = [];
...
function validateMenuTableStructure(componentContent) {
  const results = [];
...
function validatePageStructureConsistency(menuPageContent, storePageContent) {
  const results = [];
...
function runStaticValidation() {
  ...
  try {
    ...
    const menuTableContent = fs.readFileSync('app/menu/components/MenuTable.tsx', 'utf-8');
    results.push(...validateMenuTableStructure(menuTableContent));

    // Read page files
    const menuPageContent = fs.readFileSync('app/menu/page.tsx', 'utf-8');
    const storePageContent = fs.readFileSync('app/stores/page.tsx', 'utf-8');
...
function generateStaticValidationReport(results) {
...
// Run the validation
const results = runStaticValidation();
const report = generateStaticValidationReport(results);
console.log(report);