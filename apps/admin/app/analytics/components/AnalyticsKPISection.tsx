...
export default function AnalyticsKPISection({ kpis, loading }: AnalyticsKPISectionProps) {
      ...
      <section className="s-kpis">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="s-card">
            <div className="s-k">Loading...</div>
            <div className="s-v">—</div>
            <div className="s-sub">Updating data</div>
          </div>
        ))}
      </section>
    );
