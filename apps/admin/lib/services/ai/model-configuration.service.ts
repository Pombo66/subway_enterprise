...
export class ModelConfigurationManager {
  ...
  private constructor() {
    ...
    console.log('🤖 Model Configuration Manager initialized');
   ...
   */
  public static getInstance(): ModelConfigurationManager {
  ...
  getModelForOperation(operation: AIOperationType): string {
    switch (operation) {
      ...
      default:
        console.warn(`Unknown operation type: ${operation}, falling back to default`);
  ...
  validateModelConfiguration(): ModelValidationResult {
    ...
    Object.entries(this.config).forEach(([operationType, model]) => {
      ...
      // Warn about cost-effective model usage
    ...
    if (warnings.length > 0) {
      console.warn('Model configuration warnings:', warnings);
    }
  ...
  updateModelConfiguration(config: Partial<ModelConfig>): void {
    ...
    console.log(`Model configuration updated: ${JSON.stringify(config, null, 2)}`);
  }
  ...
  getModelPricing(model: string): ModelPricing {
    ...
    if (!pricing) {
      console.warn(`No pricing information available for model: ${model}`);
