...
export class CompetitiveLandscapeAssessmentService {
  ...
  constructor(private readonly prisma: PrismaClient) {
    ...
    console.log('🏆 Competitive Landscape Assessment Service initialized');
  }
  ...
  async assessCompetitiveLandscape(request: CompetitiveAnalysisRequest): Promise<CompetitiveLandscapeResult> {
    console.log(`Assessing competitive landscape for region: ${request.region}`);
    ...
    try {
      ...
      console.log(`Competitive assessment completed for ${request.region}`);
      ...
      console.error('Competitive landscape assessment failed:', error);
  ...
  private async performAICompetitiveAnalysis(
    ...
    if (!this.OPENAI_API_KEY) {
      console.warn('OPENAI_API_KEY not configured, using basic analysis');
      return {};
    }
    ...
    try {
      const response = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a competitive intelligence analyst specializing in restaurant market analysis. Provide detailed competitive assessments with actionable insights and strategic recommendations. Always respond with valid JSON.'
            },
            {
              role: 'user',
              content: prompt
      ...
      if (!response.ok) {
        console.warn(`AI competitive analysis failed: ${response.status}`);
        return {};
      }
