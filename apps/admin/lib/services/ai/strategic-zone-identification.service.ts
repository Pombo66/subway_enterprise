...
export class StrategicZoneIdentificationService {
  ...
  constructor(private readonly prisma: PrismaClient) {
    ...
    console.log('🎯 Strategic Zone Identification Service initialized');
  }
  ...
  async identifyStrategicZones(request: ZoneIdentificationRequest): Promise<StrategicZone[]> {
    console.log(`Identifying strategic zones for region: ${request.marketAnalysis.region}`);
    ...
    try {
      ...
      console.log(`Identified ${finalZones.length} strategic zones`);
      return finalZones;

    } catch (error) {
      console.error('Strategic zone identification failed:', error);
      throw new Error(`Zone identification failed: ${error.message}`);
    }
  }
  ...
  private async enhanceZoneIdentification(request: ZoneIdentificationRequest): Promise<StrategicZone[]> {
    if (!this.OPENAI_API_KEY) {
      console.warn('OPENAI_API_KEY not configured, using basic zone identification');
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a strategic expansion analyst specializing in identifying optimal zones for restaurant expansion. Focus on geographic clustering, market opportunities, and risk assessment. Always respond with valid JSON.'
          },
          {
            role: 'user',
            content: prompt
    ...
    if (!response.ok) {
      console.warn(`AI enhancement failed, using basic zones: ${response.status}`);
      return request.marketAnalysis.strategicZones;
    }
