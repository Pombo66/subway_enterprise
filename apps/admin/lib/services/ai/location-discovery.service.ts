...
export class LocationDiscoveryService {
  ...
  private readonly MAX_TOKENS = 2000; // Lower for GPT-5-nano
  ...
  constructor(private readonly prisma: PrismaClient) {
    ...
    console.log('🎯 Location Discovery Service initialized with GPT-5-nano');
  }
  ...
  async discoverLocations(request: LocationDiscoveryRequest): Promise<LocationDiscoveryResult> {
    ...
    console.log(`Starting location discovery for ${request.targetCount} candidates`);
    ...
    try {
      ...
      for (const zone of request.strategicZones) {
        ...
        console.log(`Generating ${candidatesPerZone} candidates for zone ${zone.id}`);
      ...
      console.log(`Location discovery completed: ${filteredCandidates.length} candidates in ${generationTime}ms`);
      ...
      const response = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a location discovery specialist for restaurant expansion. Generate viable location candidates within specified zones, focusing on accessibility, foot traffic, and market potential. Always respond with valid JSON.'
            },
            {
              role: 'user',
              content: prompt
